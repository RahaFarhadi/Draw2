...
namespace dxf_ext
{
    public class Class1
    {
        ...
        private static void AddLineLikeEntityMetadata(DxfDocument doc, Action<string?> add)
        {

            foreach (var line in doc.Entities.Lines)
            {
                AddEntityMetadata(line, add);
            }


            foreach (var polyline in doc.Entities.LwPolylines)
            {
                AddEntityMetadata(polyline, add);
            }
        ...
        private static void AddEntityMetadata(EntityObject entity, Action<string?> add)
        {
            add(entity.Layer?.Name);
            add(entity.Linetype?.Name);
            add(entity.Handle);
        }
